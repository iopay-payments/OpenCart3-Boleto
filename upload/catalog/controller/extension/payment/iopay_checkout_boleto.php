...
class ControllerExtensionPaymentIOPAYCheckoutBoleto extends Controller{
    ...
    public function index(){
        ...
        $data['testmode'] = $this->config->get('payment_iopay_checkout_boleto_test');

        if (!$this->config->get('payment_iopay_checkout_boleto_test')) {
            $data['action'] = 'https://checkout.iopay.com.br/app/pay/' . $this->config->get('payment_iopay_checkout_boleto_ioid');
        } else {
            $data['action'] = 'https://sandbox.checkout.iopay.com.br/app/pay/' . $this->config->get('payment_iopay_checkout_boleto_ioid');
        }
        ...
        $customer_primarydocument_id =  $this->config->get('payment_iopay_checkout_boleto_mapping_user_primarydocument');
        $shipping_number_shipping_id =  $this->config->get('payment_iopay_checkout_boleto_mapping_shipping_addressnumber');
        ...
        if ($order_info) {
            ...
            $data['business'] = $this->config->get('payment_iopay_checkout_boleto_email');
            ...
            $data['notify_url'] = $this->url->link('extension/payment/iopay_checkout_boleto/callback', '', true);
            $data['cancel_return'] = $this->url->link('checkout/checkout', '', true);

            // IO Seller ID
            $data['iopay_seller_id'] = $this->config->get('payment_iopay_checkout_boleto_ioid');
            ...
            $data['iopay_antifraud_id'] = $this->config->get('payment_iopay_checkout_boleto_antifraud_id');
            $data['iopay_antifraud_type'] = $this->config->get('payment_iopay_checkout_boleto_antifraud_type');
            ...
            if (!$this->config->get('payment_iopay_checkout_boleto_transaction')) {
            ...
            return $this->load->view('extension/payment/iopay_checkout_boleto', $data);
        }
    }
    ...
    public function send() {
        ...
        if ($order_info) {
            ...
            // payment type
            if($this->request->post['payment_type'] == 'boleto'){
            ...
            // make login to retrieve the authorization token
            $auth_token = $this->getIOPayAuthorization('transaction');
            ...
            if($custom_shipping_number != '0' && $custom_shipping_number != ''){
                ...
                $streetComplement = $order_info['payment_address_3'] != null ? $order_info['payment_address_3']:  '  ';
            ...
            if (isset($customer_response->error)) {
                ...
                $json['error'] = $customer_response->error;
                $json['error_message'] = (isset($customer_response->error->message_display) || $customer_response->error->message_display != '' ? $customer_response->error->message_display : $customer_response->error->message);
                ...
                // 2. enviamos a transação fornecendo o id do comprador e id do cartão tokenizado ou boleto
                $transactionParams = array(
                    "amount" => $amount,  // valor processado precisa estar em centavos, então multiplicamos por 100
                    "currency" => 'BRL',
                    "description" => "Compra online: " . $order_info['store_name'],
                    "statement_descriptor" => "Compra online em " . $order_info['store_name'] . " - ID: " . $order_info['order_id'],
            ...
            foreach ($this->cart->getProducts() as $product) {
                ...
                $data['products'][] = array(
                    ...
                    'amount' => $this->currency->format($product['price'], $order_info['currency_code'], false, false) * 100,
                    'quantity' => $product['quantity'],
                );
            ...
            $transaction = $this->iopayRequest($auth_token, $transactionURI, $transactionParams);

            if(isset($transaction->success)){
                $json = $this -> addSucceededTransaction($transaction, $order_info['order_id'],$this->model_checkout_order);
            }else if(isset($transaction->error)){
                $json = $this -> addFailedTransaction($transaction, $order_info['order_id'],$this->model_checkout_order);
            }else {
                print "General Error!";
            }
    ...
    public function callback(){
        ...
        if(isset($response -> success -> payment_method -> resource) && $response -> success -> payment_method -> resource  == 'card') {
            $prefix = "payment_iopay_checkout_transparente_card";
        }elseif(isset($response -> success -> payment_method -> resource) && $response -> success -> payment_type -> resource  == 'pix'){
            $prefix = "payment_iopay_checkout_transparente_pix";
        }else {
            $prefix = "payment_iopay_checkout_boleto";
        }
        ...
        if ($order_info) {
            ...
            if (!$this->config->get($prefix.'_test')) {
            ...
            if ($this->config->get($prefix.'_debug')) {
            ...
            switch ($response -> success -> status) {
                case 'canceled':
                    ...
                    $order_status_id = $this->config->get($prefix.'_canceled_status_id');
                    $notify = true;
                    break;
                case 'failed':
                    $label = "<span style='color:darkred;font-weight: bold'>FALHADO</span>";
                    $order_status_id = $this->config->get($prefix.'_failed_status_id');
                    break;
                case 'pending':
                    $label = "<span style='color:orange;font-weight: bold'>AGUARDANDO PAGAMENTO</span>";
                    $order_status_id = $this->config->get($prefix.'_pending_status_id');
                    break;
                case 'succeeded':
                    $label = "<span style='color:green; font-weight: bold'>TRANSAÇÃO APROVADA</span>";
                    $order_status_id = $this->config->get($prefix.'_processed_status_id');
                    break;
                case 'void':
                    $label = "<span style='color:darkred;font-weight: bold'>CANCELADO</span>";
                    $order_status_id = $this->config->get($prefix.'_voided_status_id');
                    break;
                default:
            ...
            if(isset($response -> success -> payment_method -> resource) && $response -> success -> payment_method -> resource  == 'boleto'){
                ...
                if ($response->success->status == 'pending' || $response->success->status == 'succeeded') {
                    ...
                    if ($response->success->status == 'pending') {
                        $status = $this->config->get('payment_iopay_checkout_boleto_pending_status_id');
                    } else if ($response->success->status == 'succeeded') {
                        $status = $this->config->get('payment_iopay_checkout_boleto_processed_status_id');
                    }else {
                        $status = $this->config->get('payment_iopay_checkout_boleto_failed_status_id');
                    }
                    ...
                    $status = $this->config->get('payment_iopay_checkout_boleto_failed_status_id');
                    if($order_status_id != $order_info['order_status_id']) {
                        $this->model_checkout_order->addOrderHistory($order_id, $status, $message, false, true); //não notifica o cliente
                    }
                }
    ...
    function addSucceededTransaction($transaction_response, $order_id, $model_checkout_order):array {
        ...
        $message = 'Processado com<br><img src="https://static.iopay.dev/assets/img/iopay.png" alt="" width="100px"><br>';
        $message .= '<hr><h4><i class="fa fa-credit-card-alt" aria-hidden="true"></i> &nbsp;<strong>Boleto Bancário</strong></h4>';

        if (isset($transaction_response->success->id)) {
            $message .= '<hr><i class="fa fa-check-circle-o"></i> &nbsp;<strong>ID da Transa&ccedil;&atilde;o:</strong><br>'.$transaction_response->success->id.' <br><br>';
        }

        $date_boleto_created = date_create($transaction_response->success->created_at);
        $message .= '<i class="fa fa-calendar-check-o"></i>&nbsp;<strong>Data da geração:</strong><br>'.date_format($date_boleto_created, "d/m/Y h:i:s").'<br><br>';


        if (isset($transaction_response->success->payment_method->payment_limit_date)) {
            $date_boleto_limit = date_create($transaction_response->success->payment_method->payment_limit_date);
            $message .= '<i class="fa fa-calendar-check-o"></i> &nbsp;<strong>Data de vencimento:</strong><br> ' . date_format($date_boleto_limit, "d/m/Y h:i:s") . "<br><br>";
        }

        if (isset($transaction_response->success->payment_method->url)) {
            $message .= '<i class="fa fa-barcode"></i> &nbsp;<strong>C&oacute;digo de Barras:</strong><br>' . $transaction_response->success->payment_method->barcode . "&nbsp;&nbsp;<i class='fa fa-external-link text-primary' onclick='window.location.href=\"".$transaction_response->success->payment_method->url."\"' ></i><br><br>";
        }

        if (isset($transaction_response->success->reference_id)) {
            $message .= '<i class="fa fa-database" aria-hidden="true"></i> &nbsp;<strong>ID de Referência:</strong><br>' . $transaction_response->success->reference_id . "";
        }

        $message .= '<h3> R$' . $transaction_response->success->amount . "</h3>\n";
        $message .= (isset($transaction_response->success->confirmed) && $transaction_response->success->confirmed == '1' ? '<span class="label label-success">PAGAMENTO CONFIRMADO' : '<span class="label label-warning">AGUARDANDO PAGAMENTO') . "</span>";
        ...
        $this->session->data['iopay_sandbox_mode'] = $this->config->get('payment_iopay_checkout_boleto_test');
        $this->session->data['iopay_total_order'] = $transaction_response->success->amount;
        $this->session->data['iopay_payment_type'] = 'boleto';

        $json['success'] = $this->url->link('checkout/success');
        $json['order_id'] = $this->session->data['order_id'];
        $json['transaction_id'] = $transaction_response->success->id;
        $json['boleto_url'] = $transaction_response->success->payment_method->url;
        ...
        if ($transaction_response->success->status == 'pending') {
            ...
            $status = $this->config->get('payment_iopay_checkout_boleto_failed_status_id');
            $model_checkout_order->addOrderHistory($order_id, $status, $message, false, true); // não notifica o cliente
    ...
    function addFailedTransaction($transaction_response, $order_id, $model_checkout_order):array{
        ...
        $json['error'] = $this->url->link('checkout/success');

        $json['iopay_request_fails'] = $this -> addRequestFail();
        if($json['iopay_request_fails'] >= 6){
            print 'Abuse!';
            exit();
        }
        ...
        if (isset($transaction_response->error->id)) {
            $message .= '<hr><i class="fa fa-check-circle-o"></i> &nbsp;<strong>ID da Transa&ccedil;&atilde;o:</strong><br>'.$transaction_response->error->id.' <br><br>';
        }

        $date_boleto_created = date_create($transaction_response->error->created_at);
        $message .= '<i class="fa fa-calendar-check-o"></i>&nbsp;<strong>Data da geração:</strong><br>'.date_format($date_boleto_created, "d/m/Y h:i:s").'<br><br>';

        if (isset($transaction_response->error->payment_method->payment_limit_date)) {
            $date_boleto_limit = date_create($transaction_response->error->payment_method->payment_limit_date);
            $message .= '<i class="fa fa-calendar-check-o"></i> &nbsp;<strong>Data de vencimento:</strong><br> ' . date_format($date_boleto_limit, "d/m/Y h:i:s") . "<br><br>";
        }

        if (isset($transaction_response->error->payment_method->url)) {
            $message .= '<i class="fa fa-barcode"></i> &nbsp;<strong>C&oacute;digo de Barras:</strong><br>' . $transaction_response->error->payment_method->barcode . "&nbsp;&nbsp;<i class='fa fa-external-link text-primary' onclick='window.location.href=\"".$transaction_response->error->payment_method->url."\"' ></i><br><br>";
        }

        if (isset($transaction_response->error->reference_id)) {
            $message .= '<i class="fa fa-database" aria-hidden="true"></i> &nbsp;<strong>ID de Referência:</strong><br>' . $transaction_response->error->reference_id . "";
        }

        $message .= '<h3> R$' . $transaction_response->error->amount . "</h3>\n";
        $message .= (isset($transaction_response->error->confirmed) && $transaction_response->error->confirmed == '1' ? '<span class="label label-success">PAGAMENTO CONFIRMADO' : '<span class="label label-warning">AGUARDANDO PAGAMENTO') . "</span>";

        $status = $this->config->get('payment_iopay_checkout_boleto_failed_status_id');

        $json['error'] = $transaction_response->error;
        $json['error_message'] = $transaction_response->error;

        if(isset($transaction_response->error->message)){
            $this->log->write('IOPAY - Falha ao gerar transação: ' . $transaction_response->error->message . ') - OrderId: '.$order_info['order_id']);
        }

        if (isset($transaction_response->error->status)) {

            switch($transaction_response->error->status){
                case 'canceled':
                    $label_status = 'NÃO APROVADO';
                    $label_badge = 'label-danger';
                    break;
                case 'failed':
                    $label_status = 'FALHADO';
                    $label_badge = 'label-danger';
                    break;
                case 'pre_authorized':
                    $label_status = 'EM ANÁLISE - AGUARDE APROVAÇÃO';
                    $label_badge = 'label-warning';
                    break;
                default :
